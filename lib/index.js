    ...
    activate() {
        ...
        this.disposable.add(atom.workspace.onDidChangeActivePaneItem(item => {
            ...
            if (editor) {
                ...
                this.activeEditor.disposable.add(selection);
            }
        }));
    }
    ...
    handleSubscriberMessage(subscriber, payload, callback = () => true) {
        switch (payload.type) {
            ...
            case 'openFile': {
                ...
                let initialLine = payload.initialLine;
                let initialColumn = payload.initialColumn;
                ...
                atom.workspace.open(filePath, {
                    initialLine,
                    initialColumn
                }).then(() => callback({ status: true }), () => callback({ status: false }));
                break;
            }
            case 'getContent': {
                let allow = false;
