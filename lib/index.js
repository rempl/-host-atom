...
class RemplClient {
    activate() {
        ...
        this.disposable.add(atom.workspace.onDidChangeActivePaneItem(item => {
            ...
            if (editor) {
                // todo onDidChangeGrammar
                // todo onDidChangePath
                let selection = editor.onDidChangeSelectionRange(sr => this.sendSelectionsInfo(sr.selection.editor));
                let addSelection = editor.onDidAddSelection(s => this.sendSelectionsInfo(s.editor));
                let removeSelection = editor.onDidRemoveSelection(s => this.sendSelectionsInfo(s.editor));

                this.activeEditor.disposable.add(selection, addSelection, removeSelection);
            }
        }));
    }
            ...
            case 'openFile': {
                ...
                if (Array.isArray(selections)) {
                    selections = selections
                        .filter(range => range.length && range[0].length)
                        .map(range => {
                            let [start, end] = range;
                            ...
                            range[0] = [+start[0] - 1, +start[1] - 1];
                            range[1] = [+end[0] - 1, +end[1] - 1];
                ...
                }.bind(null, selections), () => callback({ status: false }));
                break;
            }
            case 'getContent': {
                let allow = false;
                ...
                if (!editor || subscriber.allowToGetContent === false) {
                    return callback({ status: false });
                }

                if (!subscriber.allowToGetContent) {
                    atom.confirm({
                ...
                if (allow) {
                    callback({ status: true, content: editor.getText() });
                } else {
